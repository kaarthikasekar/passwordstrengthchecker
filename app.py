...
def check_password_strength(password, username=None, old_passwords=None, last_changed=None):
    ...
    if time_to_crack < 60:
        suggestions.append(f"Password can be cracked in {time_to_crack:.2f} seconds.So,Its strong\n")
    elif time_to_crack < 3600:
        suggestions.append(f"Password can be cracked in {time_to_crack/60:.2f} minutes.So,Its strong\n")
    elif time_to_crack < 86400:
        suggestions.append(f"Password can be cracked in {time_to_crack/3600:.2f} hours.So,Its strong\n")
    else:
        suggestions.append(f"Password can be cracked in {time_to_crack/86400:.2f} days.So,Its strong\n")
    
    # Password aging
    if last_changed:
        days_since_changed = (datetime.now() - last_changed).days
        if days_since_changed > 90:
            suggestions.append("Password should be changed every 90 days.\n")
...
def check_password():
    ...
    old_passwords = ["oldpassword1", "oldpassword2"]  # Example previous passwords, in a real case, retrieve from a database
    last_changed = datetime.now() - timedelta(days=100)  # Example last changed date
